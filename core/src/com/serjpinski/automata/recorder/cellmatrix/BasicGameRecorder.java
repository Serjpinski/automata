...
public class BasicGameRecorder implements CellMatrixGameRecorder {
  ...
  public BasicGameRecorder(int width, int height, String fileName) {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        ...
        if (value != lastCells[i][j]) {
          ...
          delta.writeBytes(ByteBuffer.allocate(5).putInt(position).put((byte) value).array());
        }
      }
    }
